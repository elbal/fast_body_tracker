...
def main():
	...
	gray_3_channel_image = np.empty((1080, 1920, 3), dtype=np.uint8)
	depth_3_channel_image = np.zeros((1080, 1920), dtype=np.uint8)
	colorized_image = np.zeros((1080, 1920, 3), dtype=np.uint8)
	...
	while True:
		...
		cv2.cvtColor(gray_image, cv2.COLOR_GRAY2BGR, dst=gray_3_channel_image)
		...
		cv2.convertScaleAbs(depth_image, alpha=0.08, dst=depth_3_channel_image)
		cv2.applyColorMap(
			depth_3_channel_image, cv2.COLORMAP_CIVIDIS, dst=colorized_image)

		cv2.addWeighted(
			gray_3_channel_image, 0.7, colorized_image, 0.3, 0, dst=combined_image)
