...
class Device:
    ...
    def start(
        self, configuration: Configuration, record=False, record_filepath="output.mkv"
    ):
    ...
    def update(self, timeout_in_ms: int = k4a_const.K4A_WAIT_INFINITE) -> Capture:
    ...
    def update_imu(self, timeout_in_ms: int = k4a_const.K4A_WAIT_INFINITE) -> ImuSample:
    ...
    def _create_handle(index: int) -> k4a_device_t:
        ...
        result_code = _k4a.K4aLib.k4a_device_open(index, ctypes.byref(device_handle))
    ...
    def _get_capture(
        self, timeout_in_ms: int = k4a_const.K4A_WAIT_INFINITE
    ) -> k4a_capture_t:
    ...
    def _get_calibration(self, depth_mode: int, color_resolution: int) -> Calibration:
        calibration_handle = k4a_calibration_t()
        result_code = _k4a.K4aLib.k4a_device_get_calibration(
            self._handle, depth_mode, color_resolution, ctypes.byref(calibration_handle)
        )
        if result_code != _k4a.K4A_RESULT_SUCCEEDED:
