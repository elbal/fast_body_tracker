...
class Transformation:
    ...
    def depth_image_to_color_camera(
        ...
        _k4a.K4aLib.k4a_transformation_depth_image_to_color_camera(
            self._handle, depth_image_object.handle(), transformed_image_object.handle()
        )
    ...
    def custom_image_to_color_camera(
        self,
        depth_image_object: Image,
        custom_image_object: Image,
        transformed_image_object: Image | None = None,
        interpolation: int = k4a_const.K4A_TRANSFORMATION_INTERPOLATION_TYPE_LINEAR,
    ) -> Image:
        if transformed_image_object is None:
            transformed_custom_handle = self._create_image_handle(
                custom_image_object.format,
                self.color_resolution.width,
                self.color_resolution.height,
            )
        ...
        _k4a.K4aLib.k4a_transformation_depth_image_to_color_camera_custom(
            self._handle,
            depth_image_object.handle(),
            custom_image_object.handle(),
            self._depth_image_object.handle(),
    ...
    def depth_image_to_point_cloud(
        ...
        calibration_type=k4a_const.K4A_CALIBRATION_TYPE_DEPTH,
    ) -> Image:
        if point_cloud_object is None:
            point_cloud_handle = self._create_image_handle(
                k4a_const.K4A_IMAGE_FORMAT_CUSTOM,
                depth_image_object.width,
