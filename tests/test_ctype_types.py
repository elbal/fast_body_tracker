...
def check_skeleton_t(skeleton_val, real_pos_array, real_quat_array,
                     real_conf_array):
    for i in range(pykinect.K4ABT_JOINT_COUNT):
        check_joint_t(skeleton_val.joints[i], real_pos_array[i],
                      real_quat_array[i], real_conf_array[i])


def check_skeleton2d_t(skeleton_val, real_pos_array, real_conf_array):
    for i in range(pykinect.K4ABT_JOINT_COUNT):
        check_joint2d_t(skeleton_val.joints2D[i], real_pos_array[i],
                        real_conf_array[i])
...
def test_skeleton_t():
    skeleton_pos_array = [rand_int(3) for i in
                          range(pykinect.K4ABT_JOINT_COUNT)]
    skeleton_quat_array = [rand_int(4) for i in
                           range(pykinect.K4ABT_JOINT_COUNT)]
    skeleton_conf_array = rand_int(pykinect.K4ABT_JOINT_COUNT)
    joints = [
        pykinect.k4abt_joint_t(skeleton_pos_array[i], skeleton_quat_array[i],
                               skeleton_conf_array[i]) for i in
        range(pykinect.K4ABT_JOINT_COUNT)]
    skeleton_val = pykinect.k4abt_skeleton_t(joints)
    check_skeleton_t(skeleton_val, skeleton_pos_array, skeleton_quat_array,
                     skeleton_conf_array)


def test_skeleton2d_t():
    skeleton2d_pos_array = [rand_int(2) for i in
                            range(pykinect.K4ABT_JOINT_COUNT)]
    skeleton2d_conf_array = rand_int(pykinect.K4ABT_JOINT_COUNT)
    joints = [pykinect.k4abt_joint2D_t(skeleton2d_pos_array[i],
                                       skeleton2d_conf_array[i]) for i in
              range(pykinect.K4ABT_JOINT_COUNT)]
    skeleton2d_val = pykinect.k4abt_skeleton2D_t(joints)
    check_skeleton2d_t(skeleton2d_val, skeleton2d_pos_array,
                       skeleton2d_conf_array)
...
def test_body_t():
    ...
    check_skeleton_t(body_val.skeleton, skeleton_pos_array,
                     skeleton_quat_array, skeleton_conf_array)
...
def test_body2d_t():
    ...
    check_skeleton2d_t(body2d_val.skeleton, skeleton2d_pos_array,
                       skeleton2d_conf_array)
...
def test_empty_values():
    ...
    check_skeleton2d_t(skeleton2d_val,
